...
#   修改日期：2022年05月11日 星期三 11时02分30秒
#   描    述：
#
#================================================================
opt_string="b:a:efd"

bin_file="build/v5_test_b.hex"
...
function main() {
	...
	if [ $erase_flash -eq 1 ];then
		...
		JLinkExe -Device "STM32F207VETx" -IF "$link_if" -JTAGConf "-1,-1" -Speed "4000" -CommanderScript "$jlink_script"
		rm "$jlink_script"
	fi
	...
	if [ $debug -eq 1 ];then
		...
		#JLinkExe -Device "STM32F207VETx" -IF "$link_if" -JTAGConf "-1,-1" -Speed "4000" -CommanderScript "$jlink_script"
		#rm "$jlink_script"
		JLinkGDBServer -select USB -device STM32F207VETx -endian little -if $link_if -speed 4000 -noir -noLocalhostOnly
	fi
}
